...
export default function buildHierarchy(json, chunkName) {
    // var modules = json.modules;
    // 'entryChunk-www'
    var chunk = json.chunks.find(
        ({ names }) => names.includes(chunkName));
    var modules = chunkName === '*' ? json.modules : json.modules.filter(
        (module) => module.chunks.includes(chunk.id));
    var maxDepth = 1;
